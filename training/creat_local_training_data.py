...
es = Elasticsearch()
yahoo = Share('DUST')
res = es.search(index='stocks',doc_type='DUST', body={ "size": 0, "aggs": { "avg_grade": { "avg": { "field": 'scoring'}}}})
...
res = es.search(index='stocks',doc_type='DUST', body={ "size": 3000, "query": { "range": { "created_at": { "gte": date_passed_48_hours, "lte": date_now}}}})
...
for data in dataset:
	...
	if dt.weekday() <5:
		...
		res = es.search(index='stocks',doc_type='DUST', body={ "size": 0, "query": { "range": { "created_at": { "gte": time_range_start, "lte": time_range_end}}}, 
		  "aggs": { "avg_grade": { "avg": { "field": 'scoring'}}}})
		score_range_avg = res["aggregations"]["avg_grade"]["value"]
		res = es.search(index='stocks',doc_type='DUST', body={ "size": 0, "query": { "range": { "created_at": {"lte": time_range_start}}}, 
		  "aggs": { "avg_grade": { "avg": { "field": 'scoring'}}}})
		...
		if score_range_avg > 0 and score_range_avg !=None and score_range_avg_pre !=None:
			...
			scoring_increase_than_pre = (score_range_avg-score_range_avg_pre)/(score_range_avg_pre if score_range_avg_pre !=0 else 1) 
